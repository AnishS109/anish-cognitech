...
const CoursesCards = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const response = await axios.get("https://anish-cognitech-backend-fwia.onrender.com/api/latest/latest-course");
          ...
          {courses.map((course, index) => (
            <Grid item xs={12} sm={6} md={4} key={index}>
              <Card sx={{ maxWidth: 345, transition: "transform 0.4s ease, box-shadow 0.2s ease", "&:hover": { transform: "scale(1.03)", boxShadow: "0 10px 16px rgba(0, 0, 0, 0.8)" } }}>
                <CardMedia sx={{ height: 160, objectFit: 'fill' }} component="img" image={staticImages[course.course_name]} alt={course.course_name} />
                <CardContent>
        ...
        {selectedCourse && (
          <Dialog open={open} onClose={handleClose} fullWidth maxWidth="sm">
            ...
            <DialogContent>
              ...
              <img src={staticImages[selectedCourse.course_name] || Python} alt={selectedCourse.course_name} style={{ width: '100%', height: 'auto' }} />
            </DialogContent>
            <DialogActions>
